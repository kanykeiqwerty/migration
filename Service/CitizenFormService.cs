...
namespace MigrationApi.Service
{
    public class CitizenFormService : ICitizenFormService
    {
        ...
        public async Task<List<CitizenFormDto>> GetAllAsync(CitizenFormFilterDto filter)
        {


            var forms = await _repo.GetAllAsync(filter);
        ...
        public async Task<List<CitizenFormDto>> GetAllActiveAsync()
        {


            var forms = await _repo.GetAllActiveAsync();
        ...
        public async Task<List<CitizenFormDto>> GetAllArchivedAsync()
        {
            var forms = await _repo.GetAllArchivedAsync();
            return forms.Select(f => new CitizenFormDto
            {
                Id = f.Id,
                RegistrationDate = f.RegistrationDate,
        ...
        public async Task<OneCitizenFormDto?> GetByPINAsync(string PIN)
        {
            var f = await _repo.GetByPINAsync(PIN);
            if (f == null) return null;
        ...
        public async Task<bool> UpdateAsync(Guid id, UpdateCitizenFormDto dto)
        {
            ...
            if (form.IsArchived)
                throw new InvalidOperationException("Archived records cannot be modified.");
        ...
        {
            var citizen = await _repo.GetByIdAsync(id);
            if (citizen == null) return false;

            if (!citizen.IsArchived)
                throw new InvalidOperationException("Citizen form is already active.");

            citizen.IsArchived = false;
        ...
        public async Task<bool> ArchiveAsync(Guid id)
        {
            ...
            if (citizen.IsArchived)
                throw new InvalidOperationException("Citizen form is already unactive.");

            citizen.IsArchived = true;
            await _repo.UpdateAsync(citizen);
            await _repo.SaveAsync();
            return true;
        }
